...
def ask_names_pygame():
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if input_rect_blancas.collidepoint(event.pos):
                    ...
                    if (
                        not nb
                        or nb.lower() == PLACEHOLDER_WHITE.lower()
                        or not nn
                        or nn.lower() == PLACEHOLDER_BLACK.lower()
                    ):
                        error_msg = (
                            "Ingresá nombres reales: no se permiten 'Jugador 1' ni 'Jugador 2'."
                        )
                    else:
                        return nb, nn
                else:
        ...
        pygame.draw.rect(
            screen,
            color_active if active_field=='blancas' else color_inactive,
            input_rect_blancas,
            2,
            border_radius=6,
        )
        pygame.draw.rect(
            screen,
            color_active if active_field=='negras' else color_inactive,
            input_rect_negras,
            2,
            border_radius=6,
        )
...
def main():
    ...
    try:
        ...
        ui.run()  # pylint: disable=no-member
    finally:
        pygame.quit()
